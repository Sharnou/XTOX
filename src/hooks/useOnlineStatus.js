...
export function useOnlineStatus(targetEmail) {
  ...
  const checkStatus = async (email) => {
    const users = await XTOX.entities.User.filter ? 
      await XTOX.entities.User.list("-updated_date", 100).then(all => all.filter(u => u.email === email)) :
      [];
...
export function useUpdatePresence() {
  ...
  useEffect(() => {
    if (!user) return;
    const update = () => XTOX.auth.updateMe({ last_seen: new Date().toISOString() }).catch(() => {});
    update();
